 ...
 * result of the customer registraton. It computes the web response 
...
registrationStatus.statusTypes = {
    ...
    'accepted'        : 202, // Valid accounts that fail to load. 
...
registrationStatus.baseMessage = {
    ...
    204 : "Hello? We didn't get anything.", // reject 
    206 : "Some required fields are missing", // rejected account missing required fields. 
    ...
    500 : "It's not you, it's me... somethings not right at our end.", // ILS unavailable (or something but let’s not get specific). 
    501 : "Um, we don't do that (yet)."     // Unfinished code-subs (under-construction).
};


registrationStatus.headerCode = 200;

// The list of messages to be sent back to the service caller.
registrationStatus.messages = [];
registrationStatus.partnerName = "";

registrationStatus.getName = function(){
    return registrationStatus.partnerName;
};
 ...
 * for the caller to know. The stuff you sent is broken in some fundimental
 ...
 */
registrationStatus.getMessages = function() {
...
registrationStatus.setStatus = function(errorType,message="") {
    ...
    // console.log('==========>',errorType,this.statusTypes[errorType]);
...
/**
 * Sets the name of the partner organization for reporting.
 * @param {*} name string name of organization.
 */
registrationStatus.setName = function(name){
