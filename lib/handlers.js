...
// Routes object
const handlers = {};
const learningPassVersion = "v2.0.00";
...
handlers.register = function(data, callback){
    ...
    if (data.method === 'post') {
        ...
        if (data.headers["x-api-key"]) {
            ...
            if (hasDictData(json)) {
                ...
                regStatus = customerHelper.createAccounts(key,json);
            } else {
    ...
    callback(regStatus.getStatus(), 
     {"status" : regStatus.getStatus(), "message" : regStatus.getMessages(true)});
};
