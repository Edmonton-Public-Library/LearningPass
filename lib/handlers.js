...
// Routes object
const handlers = {};
const learningPassVersion = "v2.3.00";

handlers.checkDuplicate = function(data, callback){
    if (environment.useTestMode()) {
        logger.debug(`duplicate user check request: '${JSON.stringify(data)}'`);
    }
    ...
    if (data.method === 'get') {
        ...
        if (data.headers["x-api-key"]) {
            ...
            if (hasDictData(json)) {
                regStatus = customerHelper.checkDuplicate(key,json);
            } else if (hasArrayData(json)) {
                let allResponses = [];
                json.forEach(customer => {
                    let rs = customerHelper.checkDuplicate(key,customer);
