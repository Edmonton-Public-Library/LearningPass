/**
 * This file barcodes.js contains utility functions for managing
 * library barcodes.
 * 
 ...
 * expectations, but more complex scenarios are also possible.
 * 
 * @example 
 ...
 * be generated from another source.
 * 
...
barcodeHelper.getLibraryBarcode = (error,customer) => new Promise((resolve,reject) => {
    if (!error || !customer) {
        reject({"Error" : "Required params missing."});
    }
 ...
 */
 barcodeHelper.getBarcode = function(min,max,str,prefix=''){
    // if (min && max && str){
    if (str){
        ...
        if (util.hasPosIntData(max) && max > _barcode.minWidth && max > min){
            ...
            logger.warn(`Warning illegal value for barcode max width '${max}'. Using defaults: ${_barcode.maxWidth}.`);
            myMax = _barcode.maxWidth;
        }
        if (util.hasPosIntData(min) && min >= _barcode.minWidth && min < max){
            myMin = min;
        } else {
            logger.warn(`Warning illegal value for barcode min width '${min}'. Using defaults: ${_barcode.minWidth}.`);
            myMin = _barcode.minWidth;
        }
 
        str = str.trim().toUpperCase();
        // Handle prefixes and pad out barcodes so they fit the max width.
        if (prefix.length > 0) {
            let targetLength = myMax - prefix.length;
            let myBarcode = targetLength > str.length ? str.padStart(targetLength, '0') : str;
            str = `${prefix}${myBarcode}`;
        }        

        let barcodeStrictRx= new RegExp(`^\\w{${myMin},${myMax}}$`, 'gm');
        // Clean trailing white space and check against just numbers 6,100
        let bcArr = str.match(barcodeStrictRx);
        if (bcArr != null && bcArr.length > 0){
            let potentialBarcode = bcArr[0];
            if (potentialBarcode.length >= myMin && potentialBarcode.length <= myMax){
