...
/**
 * Creates multiple accounts from JSON array of customer registration data.
 * 
 * @param {*} apiKey partner organization's API key.
 * @param {*} customers array.
 * @returns response object.
 */
customerHelper.createAccounts = function(apiKey,customers) {
    let response = registrationStatus();
    if (utils.hasArrayData(customers)) {
      customers.forEach(customer => {
        let res = customerHelper.createAccount(apiKey,customer);
        response.setStatus('success', res.getMessages(true));
      });
    } else {
...
customerHelper.getDOB = function(str,partnerConfig){
  if (dateHelpers.hasDateData(str)){
    ...
    if (daysAndYears.years >= 0) { 
      ...
      logger.error(`Error, customer birthday (${str}) is in the future.`);
    }
  } else {
    logger.error(`Error, "${str}" is not a valid birthdate.`);
  }
  return '';
};
