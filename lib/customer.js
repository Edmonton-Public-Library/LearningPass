...
const regStatus = require('./response');
 ...
 */
customerHelper.createAccount = (regStatus,apiKey,rawData) => new Promise((resolve,reject) => {
  ...
  customerHelper.validate(regStatus,rawData,pc)
...
customerHelper.clean = function(key,value,pc){
  ...
  if (key) {
    ...
    // @TODO move to initial processing instructions
 ...
 */
customerHelper.mergeFields = (customer,partnerConfig) => new Promise((resolve,reject) => {
 ...
 * valid data.
 * @param {*} regStatus The customer errors object.
 ...
 */
customerHelper.validate = (regStatus,customer,partnerConfig) => new Promise((resolves, reject) => {
  if (!regStatus){
  ...
  if (utils.hasArrayData(allRequired)){
    for (let i = 0; i < allRequired.length; i++){
      ...
      if (utils.hasStringData(cleanData)) {
        ...
        regStatus.brokenFields.push(`${field}`);
      }
    }
    // If there were problems above stop account creation process.
    if (regStatus.brokenFields.length > 0) {
      regStatus.messages.push(`${libraryCustomerSettings.library} registration requires valid data in these fields:`);
      regStatus.brokenFields.forEach(field => {
        regStatus.messages.push(`"${field}" `);
      });
      reject(regStatus);
    }
  } 
 ...
 */
customerHelper.checkOptionalFields = (customer,partnerConfig) => new Promise((resolve,reject) => {
  if (!customer){ reject(`${customerHelper._msg.noCustomer} : checkOptionalFields`); }
