...
customerHelper.clean = function(key,value,pc){
  ...
  if (key) {
    ...
    if (key === 'pin') return customerHelper.getPassword(value,pc);
 ...
 */
customerHelper.getPassword = function(str,partnerConfig){
  ...
  let passCfg = myPCMap.get('passwords');
...
customerHelper.getBarcode = function(str,partnerConfig){
  let myPCMap = new Map(Object.entries(partnerConfig));
  if (myPCMap.has('barcodeFilter')) {
    /** @TODO remove after testing. */
    // for (let [key, value] of myPCMap.entries()) {
    //   console.log('----------->' + key + ' = ' + value);
    // }
    let min = myPCMap.get('barcodeFilter').minimum;
    let max = myPCMap.get('barcodeFilter').maximum;
    let prefix = myPCMap.get('barcodeFilter').prefix;
    /**
    ...
    */
    ...
    if (helpers.hasStringData(verifiedBarcode)){
      ...
      console.log(`Error, barcode '${str}' + prefix (if any) exceed 'barcodeFilter' specs.`);
      return '';
    }
