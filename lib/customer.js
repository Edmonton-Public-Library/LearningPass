...
customerHelper.createAccount = function(response,apiKey,customer) {
  ...
  // create flat file.
  flat.toFlat(customer,flatCustomer,pc.flatDefaults)
    .then((flatCust) => {
      let filePath = environment.getFlatDir();
      let fileName = filePath + '/' + this._getFileName(flatCust.json.barcode);
      flat.write(flatCust,fileName)
        ...
        .catch((err) => {
          logger.error("Error writing flat data: ",err);
  ...
  if (utils.hasArrayData(allRequired)){
    for (let i = 0; i < allRequired.length; i++){
      ...
      if (utils.hasStringData(cleanData)) {
        ...
        field === 'pin' ? response.setStatus('incomplete',`${field} is invalid`) : response.setStatus('incomplete',`${field}`);
      }
    }
  }
};
