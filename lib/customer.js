...
customerHelper.getCountry = function(str,partnerConfig){
  if (helpers.hasStringData(str.trim())){
    return helpers.capitalize(str.trim());
  } else {
...
customerHelper._getFromDefault = function(str,partnerConfig){
  ...
  if (helpers.hasDictData(partnerConfig) && helpers.hasDictData(partnerConfig.defaults)){
    ...
    if (helpers.hasDictData(_defaults)) {
      if(helpers.hasStringData(_defaults[str.trim()])){
        return _defaults[str.trim()];
      } 
    }
  }
...
customerHelper.validate = (custErr,cust,required) => new Promise((resolves, rejects) => {
  ...
  for (i = 0; i < requiredList.length; i++){
    ...
    let cleanData = customerHelper.clean(field,cust[field],required);
    if (cleanData) {
