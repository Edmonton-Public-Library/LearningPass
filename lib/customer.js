...
customerHelper.createAccount = function(response,apiKey,customer) {
  ...
  let promisePreprocess = new Promise((resolve,reject) => {
    this.checkNoteTokens(customer,pc);
    resolve("pre-process customer complete");
  });
  ...
  let promiseValidate = new Promise((resolve,reject) => {
    ...
    if (response.hasErrors()) {
      resolve(`Error ${response.getName()} send bad data}`);
    }
    resolve("validate complete");
  }); 

  // eslint-disable-next-line no-unused-vars
  let promiseMerge = new Promise((resolve,reject) => {
    // Merge fields, like 'City, Province' if specified in configuration.
    this.mergeFields(customer,pc);
    resolve("merge complete");
  });
      
  // eslint-disable-next-line no-unused-vars
  let promiseCompileCustomer = new Promise((resolve,reject) => {
    // create flat file.
    ...
    flat.toFlat(customer,flatty,pc.flatDefaults)
      ...
      .catch((err) => {
        ...
        resolve("Error in registration ",err);
      });
  });

  // eslint-disable-next-line no-unused-vars
  let promiseWriteFlat = new Promise((resolve,reject) => {
    ...
    flat.write(flatCust,fileName)
      .then((fileName) => {
        ...
        resolve(console.log(`Wrote to "${fileName}"`));
      })
      .catch((err) => {
        response.setStatus('internalError');
        resolve("Error writing flat data: ",err);
      });
  });
