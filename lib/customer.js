...
customerHelper._msg = { 
  noCustomer : "customer not found ",
  missingRequireField : "missing required field"
};
...
customerHelper.createAccount = (custErr,apiKey,rawData) => new Promise((resolves,rejects) => {
  ...
  customerHelper.validate(custErr,rawData,pc)
...
/**
 * @TODO call this after the rest of the formatting and before the flat.toFlat()
 * get called.
...
customerHelper.checkNoteTokens = function(str,partnerConfig){
  ...
  if (!utils.hasDictData(notesObject)) {
    return utils.hasStringData(str) ? str : '';
  } else {
    const resultObj = {};
    resultObj.result = '';
    resultObj.errors = '';
...
/**
 * The arg customer's gender field checked and mapped to the 'genderMap' dictionary.
 * Comparisons are case-insensitive, that is 'male' = 'MALE'.
 * Returned strings are in the case they are defined as in partner.json.
 * 
 * @param {*} customer A string value of gender.
 * @param {*} partnerConfig A dictionary of organization's gender definitions,
 * as the keys to the library's value.
 * @example of "genderMap": { "MALE" : "M", "FEMALE" : "F", "NA" : "NA", "NONE" : "X" }
 ...
 */
customerHelper.getGender = function(customer,partnerConfig,isRequired=false){
  return new Promise((resolve,reject) => {
    if (!customer.gender || !utils.hasStringData(customer.gender)){
      // Did not receive gender from the partner, but did they
      // or the library request a default?
      customer['gender'] = customerHelper._getFromDefault("gender",partnerConfig);
      resolve(customer);
    } 
    // Get the partner's genderMap from their config, and do a lookup
    // for the passed-str. But genderMap is optional.
    let gMap = customerHelper._getDict('genderMap',partnerConfig);
    if (utils.hasDictData(gMap)){
      let gender = customer['gender'].toUpperCase();
      if (utils.hasStringData(gMap[gender])){
        customer['gender'] = gMap[gender];
        resolve(customer);
      } else {
        customer['gender'] = '';
      }
    ...
    if (isRequired) reject(`${customerHelper._msg.missingRequireField}:"gender"`);
  });
}; 
 ...
 */
customerHelper.mergeFields = (customer,partnerConfig) => new Promise((resolve,rejects) => {
...
customerHelper.validate = (custErr,customer,partnerConfig) => new Promise((resolves, rejects) => {
  ...
  // else {
  //   // This is a configuration error requiring addressing before server runs.
  //   throw new Error('Error: no fields were marked as required.');
  // }


  // @TODO move out to a promise.
  // Merged fields may be required depending on your ILS.
  customerHelper.mergeFields(customer,partnerConfig);
 ...
 */
customerHelper.checkOptionalFields = (customer,partnerConfig) => new Promise((resolve,rejects) => {
  if (!customer){ rejects(`${customerHelper._msg.noCustomer} : checkOptionalFields`); }
