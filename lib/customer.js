...
customerHelper.createAccount = function(response,apiKey,customer) {
  ...
  flat.toFlat(customer,flatty)
    .catch(console.log);

  /** @TODO compute real file name */
  let filePath = environment.getFlatDir();
  let fileName = filePath + '/' + customer.barcode + '.flat';
  flat.write(flatty,fileName)
    .then((fileName) => {
        console.log(`Wrote to "${fileName}"`);
    })
    .catch((err) => {
      console.log(`Error writing to "${err.errors}"`);
      console.log(`${err.data}`);
    });

  // Add any errors from the flat process to the response.
