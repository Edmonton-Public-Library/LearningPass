...
/**
 * Uses a local database to test if a customer already has an account.
 * @param {*} apiKey Institution's API key, proof they can use this service.
 * @param {*} customer customer data in JSON format. See documentation.
 */
customerHelper.checkDuplicate = function(apiKey,customer) {
  ...
  // Do we use duplicate check even?
  if (this.partnerUsesDuplicateChecking()) {
    response.setStatus('success', {'stdout' : ['0'], 'stderr' : []});
  }
  // Some of these may be empty, but all _should_ be defined.
  // Guard for missing values anyway, and lower case them all to 
  // match stored values in the database.
  // The keys are inconsistent with other naming conventions but match
  // the keys used by OLR's duplicate check expectations.
  let firstName = utils.getDataByKey(customer,'fname').toLowerCase();
  let lastName  = utils.getDataByKey(customer,'lname').toLowerCase();
  let email     = utils.getDataByKey(customer,'email').toLowerCase();
  // You never know what will end up in this field...
  let dob       = utils.getDataByKey(customer,'dob').toLowerCase();

  /**
   * The preferred positive matches are:
   * email, fname, lname, dob
   * email, dob, lname
   * email, dob, fname
   * dob, fname, lname
   * 
   * If they don't match on these tests, they are (probably) don't
   * have an account yet.
   */
  // Array of the OS command parameters.
  let flags = [];
  if (environment.useTestMode()) {
    response.setStatus('success', utilityTests.systemCmd('false',flags));
  } else {
    if (dob !== '') {
      flags.push('-d');
      flags.push(dob);
      ...
      // make the call to the os script 'isDuplicate' (check_user.sh, or what ever you are using).
      let result = utilityTests.systemCmd('isDuplicate',flags,false);
      // console.log('===',result);
      if (result.stdout.length > 0) {
        response.setStatus('success',result.stdout);
      } else if (result.stderr.length > 0) {
        console.log('duplicate check error ',result.stderr);
        response.setStatus('internalError','check duplicate error');
      } else {
        console.log('duplicate check issue: service not found.');
        response.setStatus('noContent');
      }
    } else {
      // Since all queries require at least 'dob' if it isn't present, we'll return -1 as
      response.setStatus('incomplete','duplicate check requires date of birth minimally.');
    }
  }
  return response;
}
...
/**
 * If a library defines 'checkDuplicates' in their config.json all accounts are 
 * checked against the ILS to ensure the account is not a duplicate. If the 
 * library sets it to false, duplicate checking is left up to the partner organization
 * to decide. If the switch is missing completely no duplicate checking is done, 
 * and it does exist, the partner's setting is honoured. 
 * 
 * Bottom line; the library's setting of 'true' trumps any partner preference.
 * @param {*} partnerConfig 
 * @returns true if the given partner or library requires checking for duplicates
 * and false otherwise.
 */
customerHelper.partnerUsesDuplicateChecking = function(partnerConfig) {
  let checksForDuplicates = environment.checkForDuplicates();
  if (typeof(checksForDuplicates) === 'undefined') {
    if (partnerConfig) {
      return typeof(partnerConfig.checkDuplicates) === 'boolean' ? partnerConfig.checkDuplicates : false;
    } 
  } else {
    return checksForDuplicates;
  }
}
...
customerHelper.clean = function(key,value,pc){
  ...
  if (key) {
    ...
    if (key === 'middleName') return helpers.getMiddleName(value);
    ...
    if (key === 'careOf') return customerHelper.getCareOf(value,pc);
  }
  return '';
};
