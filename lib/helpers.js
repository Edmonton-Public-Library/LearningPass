...
/**
 * Ensures name is not ','-separated and returns the middle name in upper case.
 * Strips anything other than word chars.
 * @param {*} str name string. 
 */
 helpers.getMiddleName = function(str){
    // False means get the last word which is the first name in a comma-sep'd string.
    if (str) {
        return str.indexOf(',') > -1 ? this.splitCommaString(str,false).toUpperCase() : this.splitCommaString(str,true).toUpperCase();
    }
    return '';
}
