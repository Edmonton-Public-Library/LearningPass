...
const helpers = {};
const _field = {};
_field.emailRx      = new RegExp("/^[_A-Z0-9-+]+(\\.[_A-Z0-9-]+)*@[A-Z0-9-]+(\\.[A-Z0-9]+)*(\\.[A-Z]{2,})$/gmi", "gmi");
...
_field.barcodeStrict= new RegExp("\\d{6,14}");
_field.barcodeLoose = new RegExp("[A-Z0-9_-]{5,14}", "gmi");
...
/**
 * Returns the barcode if str is between 'n' and 'm' chars in length.
 * @param {*} min Min length of barcode
...
helpers.getBarcode = function(min,max,str){
    if (min && max && str){
        ...
        if (bcArr != null && bcArr.length > 0){
            ...
            if (maybe_number.length() >= n && maybe_number.length() <= m){
                return typeof(maybe_number) == 'number' ? maybe_number : '';
            } else {
                return '';
            }
        } else {
