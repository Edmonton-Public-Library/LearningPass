...
_field.passwordRx     = new RegExp('^.{4,125}(?!\w)', 'gmi');
_field.dateRx         = new RegExp('[12][09][0-9][0-9][.\/-][0|1][0-9][.\/-][0-3][0-9]', 'gm');

/**
 * Checks if a date is present in the supplied string and cleans it.
...
helpers.getDate = function(str){
    if (!str) {
        ...
        let date = str.trim().replace(/\//g,'');
        date = date.replace(/-/g,'');
        // console.log("==========>",date);
        let dateArr = date.match(/[12][09][0-9][0-9][0|1][0-9][0-3][0-9]/g);
...
helpers.isDate = function(str){
    ...
    return date.getFullYear() + (date.getMonth()+1) && dd === date.getDate() && yyyy === date.getFullYear();
