...
helpers.hasDateData = function(str){
    if (helpers.hasStringData(str)){
        ...
        if (str.match(_field.ansiDateRx)){
            ...
            let mm   = parseInt(parms.slice(4,6).join(''),10);
            let dd   = parseInt(parms.slice(6).join(''),10);
            testDate = new Date(yyyy,mm-1,dd,0,0,0,0);
        } else {
            testDate = new Date(str);
        }
...
/**
 * Computes how many days and years ago a given date was.
 * @param {*} testDate 
 * @returns {days, years} asscociative array. (-) days, years
 * in the past, (+) days, years from now.
 */
helpers.daysYearsFromToday = function(testDate) {
    ...
    let daysYears = {};
    let days = sign + Math.round(elapsed/msPerDay);   
    let years= sign + Math.round(elapsed/msPerYear);
