 ...
 * these values.
*/
flat.defaults = new Map();
flat.defaults.set("USER_NAME_DSP_PREF","0");
flat.defaults.set("USER_PREF_LANG"    ,"ENGLISH");
flat.defaults.set("USER_ROUTING_FLAG" ,"Y");
flat.defaults.set("USER_CHG_HIST_RULE","ALLCHARGES");
flat.defaults.set("USER_ACCESS"       ,"PUBLIC");
flat.defaults.set("USER_ENVIRONMENT"  ,"PUBLIC");
flat.defaults.set("USER_MAILINGADDR"  ,"1");
flat.defaults.set("NOTIFY_VIA"        ,"PHONE");
flat.defaults.set("RETRNMAIL"         ,"YES");
...
flat._updateDefaults = function(defaultMap, defaults){
    ...
    if (utils.hasDictData(defaults)) {
        ...
        for (let [key,value] of argDefaultMap){
            ...
            if (flat.tagMap.has(key)) {
                myDefaults.set(key,value);
            } else {
                console.log(`Error requested default "${key}" is not a valid Symphony field.`);
            }
        }
    }
    return myDefaults;
...
flat.toFlat = (jsonData,flatData,defaults={}) => new Promise((resolve,reject) => {
    ...
    if (!utils.hasDictData(flatData)) {
        reject(new Error(flat._msg.noFlatContainer));
    }
    let flatCustomer = flatData;
    if (!utils.hasDictData(jsonData)) {
        flatCustomer.errors.push(flat._msg.noJson);
        reject(flatCustomer);
    }
    
    
    // clear the data out of the addr1 and xinfo
    addr1.clear();
    xinfo.clear();
    
    
    // Now map the incoming field to the Symphony tag, and convert dates etc.
    let customerDataMap = new Map(Object.entries(jsonData));
    for(const [key,value] of customerDataMap.entries()) {
        let nonBlockData = flat._getBlockData(key,value);
        if (nonBlockData) {
     ...
     */
    let myDefaults = flat._updateDefaults(flat.defaults, defaults);
    if (myDefaults.size > 0) {
        for (var [key,value] of myDefaults) {
    ...
    if (addr1.size > 0) {
        ...
        for(var [key,value] of addr1.entries()) {
            flatCustomer.data.push(`.${key}.    |a${value}`);
        }
    ...
    if (xinfo.size > 0) {
        ...
        for(var [key,value] of xinfo.entries()) {
            flatCustomer.data.push(`.${key}.    |a${value}`);
        }
