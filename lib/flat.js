...
flat._msg = {
    ...
    missingFlatData: 'No flat data to write flat file.',
...
flat.toFlat = (jsonData,flatData,defaults={}) => new Promise((resolve,reject) => {
     ...
     */
    let myDefaults = flat._updateDefaults(flat._defaults, defaults);
 ...
 * @param {*} error where to return any errors.
...
flat.write = (flatData,fileName=false) => new Promise((resolve,reject) => {
    ...
    if (!flatData) {
        console.log(flat._msg.missingFlatData);
        return false;
    }
    ...
    if (fileName){
        ...
        fs.open(fileName + '.flat', 'w', function(err,fileDescriptor){
            if (!err && fileDescriptor) {
                for (let i = 0; i < flatData.data.length; i++) {
                    var stringData = flatData.data[i];
                    // write to the file and close it.
                    fs.writeFile(fileDescriptor, stringData, function(err){
                        if(!err){
                            fs.close(fileDescriptor, function(err){
                                if(err){
                                    flatData.errors.push(flat._msg.errorFileClose);
                                    reject(err);
                                }
                            });
                        } else {
                            flatData.errors.push(flat._msg.errorFileWrite);
                            reject(err);
                        }
                    });
                } // End for loop.
            } else {
                flatData.errors.push(flat._msg.errorFileOpen);
                reject(err);
            }
        });
    } else {
        ...
        // Write the data to command line.
        for (let i = 0; i < flatData.data.length; i++) {
            console.log(`${flatData.data[i]}`);
        }
    }
    resolve(`flat file out put to ${fileName}.`);
});
