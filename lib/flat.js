    ...
    if (utils.hasDictData(defaults)) {
        ...
        for (let [key,value] of argDefaultMap){
            ...
            if (symphonyTags[key]) {
                ...
                logger.error(`${this._msg.invalidSymphonyTag}: "${key}".`);
            }
        }
    }
    return myDefaults;
...
flat.toFlat = (jsonCustomer,flatCustomer,flatDefaults={}) => new Promise((resolve,reject) => {
    ...
    // addr2 = {};
    if (addr2.size > 0) {
        flatCustomer.data.push(`.${symphonyTags.USER_ADDR2_BEGIN}.`);
        for(let [key,value] of addr2.entries()) {
            flatCustomer.data.push(`.${key}.   |a${value}`);
        }
        flatCustomer.data.push(`.${symphonyTags.USER_ADDR2_END}.`);
    }
    // addr3 = {};
    if (addr3.size > 0) {
        flatCustomer.data.push(`.${symphonyTags.USER_ADDR3_BEGIN}.`);
        for(let [key,value] of addr3.entries()) {
            flatCustomer.data.push(`.${key}.   |a${value}`);
        }
        flatCustomer.data.push(`.${symphonyTags.USER_ADDR3_END}.`);
    }
    // xinfo = {};
    if (xinfo.size > 0) {
...
flat.write = (flatCustomer,fileName=false) => new Promise((resolve,reject) => {
    ...
    if (fileName){
        ...
        // But removing the new line means the next customer's DOCUMENT string is
        // concatenated to the end of the last string making reloading fail if required.
        ...
        for (let i = 0; i < flatCustomer.data.length; i++) {
            logger.info(`${flatCustomer.data[i]}`);
        }
        resolve(fileName);
    }
});
