/**
 * This file date.js contains utility functions for handling dates in javascript.
 * 
...
/**
 * Converts a javascript Date object into a string.
 * @param {*} dateObj Javascript Date object.
 * @returns string version of the date.
 */
dateHelpers.dateToString = function(dateObj) {
    if (this.hasDateData(dateObj)) {
...
dateHelpers.getDateFromDays = function(daysInt){
    if (dateHelpers.isInt(daysInt)) {
        ...
        logger.error(`Error, request for '${daysInt}' days from now is invalid.`);
        return '';
    }
};
